        ...
        if(contieneclave_aux(raiz,k) == 1 )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    raiz->derecho=indefinir_aux(raiz->derecho,nodominimo->clave);
                }
            }
        }
    }
...
NodoABBD* diccionario::intersecarcon_aux(NodoABBD* raiz1, const NodoABBD* raiz2)
{
    if( raiz1 != NULL && raiz2 != NULL)
    {
        ...
        {
            raiz1->derecho=intersecarcon_aux(raiz1->derecho,raiz2);
            raiz1->izquierdo=intersecarcon_aux(raiz1->izquierdo,raiz2);
        }
    }
    return raiz1;
}
...
{
    return( this->cantidad==d.cantidad && this->claves().iguales(d.claves()) && this->valores().iguales(d.valores()) );
}
...
ostream& operator<<(ostream& f,const diccionario& d)
{
    ...
    if(d.cantidad != 0)
    {
        f<<"[("<< l_claves.iesimo(0)<<","<<l_valores.iesimo(0)<<")";
    }
    for( int i=1;i<d.cantidad;i++)
    {
        f<<";("<< l_claves.iesimo(i)<<","<<l_valores.iesimo(i)<<")";
    }
    f<<']';
